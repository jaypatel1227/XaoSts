...
import type { FractalCanvasController, FractalCanvasControllerOptions } from "../types";
...
function cloneFractalConfig(fractal: FractalConfig): FractalConfig {
  return {
    symmetry: { ...fractal.symmetry },
    region: {
      center: { ...fractal.region.center },
      radius: { ...fractal.region.radius },
      angle: fractal.region.angle
    },
    z0: { ...fractal.z0 },
    maxiter: fractal.maxiter,
    bailout: fractal.bailout,
    palette: fractal.palette.slice(),
    formula: fractal.formula
  };
}

export function createCanvasFractalController(
  canvas: HTMLCanvasElement,
  options: FractalCanvasControllerOptions
): FractalCanvasController {
  const initialFractal = cloneFractalConfig(options.fractal);
  const state: ControllerState = {
    started: false,
    stopped: false,
    frameRequest: null,
    lastFrameTime: null,
    renderMode: options.renderMode ?? "approximation",
    fractal: cloneFractalConfig(options.fractal),
    pointer: {
      x: 0,
      y: 0,
      oldx: 0,
  ...
  const baseController: ZoomController = {
    start(): void {
      if (state.started) {
        return;
      }
    ...
    setFractal(config: FractalConfig): void {
      state.fractal = cloneFractalConfig(config);
      render();
    },
  ...
  return {
    ...
    capturePng(type = "image/png", quality?: number): string {
      return canvas.toDataURL(type, quality);
    },

    resetView(nextFractal?: FractalConfig): void {
      const resolved = nextFractal ? cloneFractalConfig(nextFractal) : cloneFractalConfig(initialFractal);
      state.fractal = resolved;
      render();
    },
